<script lang="ts">
	...
	type LandingCat = { id: string; name: string; description?: string; image?: string; order: number; active: boolean };
	type LandingProd = { id: string; name: string; description?: string; image?: string; price?: number; category: string; order: number; active: boolean; expand?: { category?: LandingCat } };

	const landingCategories = $derived((data.landingCategories ?? []) as unknown as LandingCat[]);
	const landingProducts = $derived((data.landingProducts ?? []) as unknown as LandingProd[]);

	let selectedCatId = $state('');
	const visibleProducts = $derived(
		selectedCatId ? landingProducts.filter(p => p.category === selectedCatId) : landingProducts
	);
	const hasCatalog = $derived(landingCategories.length > 0 && landingProducts.length > 0);

	function getCatImageUrl(cat: LandingCat): string {
		if (!cat.image) return '';
		return `${PB_URL}/api/files/landing_categories/${cat.id}/${cat.image}?thumb=400x300`;
	}
	function getProdImageUrl(prod: LandingProd): string {
		if (!prod.image) return '';
		return `${PB_URL}/api/files/landing_products/${prod.id}/${prod.image}?thumb=400x300`;
	}
...
<nav class="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-lg border-b border-stone-100 shadow-sm">
	<div class="max-w-6xl mx-auto px-4 sm:px-8 flex items-center justify-between h-16">
		...
		<div class="hidden md:flex items-center gap-8">
			...
			{#if hasCatalog}
				<a href="#catalogo" class="text-sm font-medium text-stone-600 hover:text-[#8B5E3C] transition-colors">Catálogo</a>
			{/if}
	...
	{#if menuOpen}
		<div class="md:hidden border-t border-stone-100 bg-white px-4 pb-4 pt-2 space-y-1">
			{#each [['#inicio','Inicio'],['#nosotros','Nosotros'],['#galeria','Galería'], ...(hasCatalog ? [['#catalogo','Catálogo']] : []),['#contacto','Contacto']] as [href, label]}
...
<section id="catalogo" class="py-20 bg-white">
	<div class="max-w-6xl mx-auto px-4 sm:px-8">
		<div class="text-center mb-12">
			<p class="text-[#8B5E3C] text-sm font-semibold tracking-widest uppercase mb-2">Nuestros productos</p>
			<h2 class="text-3xl sm:text-4xl font-bold text-[#2C2018]">Catálogo</h2>
			<div class="w-16 h-1 bg-[#D4A853] mx-auto mt-4 rounded-full"></div>
		</div>

		<!-- Category filter pills -->
		<div class="flex flex-wrap gap-2 justify-center mb-10">
			<button onclick={() => (selectedCatId = '')}
				class="px-4 py-2 rounded-full text-sm font-medium transition-all {selectedCatId === '' ? 'bg-[#8B5E3C] text-white shadow-md' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}">
				Todos
			</button>
			{#each landingCategories as cat}
				<button onclick={() => (selectedCatId = cat.id)}
					class="px-4 py-2 rounded-full text-sm font-medium transition-all {selectedCatId === cat.id ? 'bg-[#8B5E3C] text-white shadow-md' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}">
					{cat.name}
				</button>
			{/each}
		</div>
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
			{#each visibleProducts as prod}
				<div class="group bg-white rounded-2xl border border-stone-200 overflow-hidden hover:shadow-lg transition-all duration-300">
					<!-- Product image -->
					<div class="relative aspect-[4/3] bg-stone-100 overflow-hidden">
						{#if prod.image}
							<img src={getProdImageUrl(prod)} alt={prod.name}
								class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
						{:else}
							<div class="w-full h-full flex items-center justify-center">
								<svg class="w-16 h-16 text-stone-300" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
							</div>
						{/if}
						<!-- Category badge -->
						<span class="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-[#8B5E3C] text-xs font-semibold px-3 py-1 rounded-full shadow-sm">
							{prod.expand?.category?.name ?? ''}
						</span>
					</div>
					<!-- Product info -->
					<div class="p-5">
						<h3 class="font-bold text-[#2C2018] text-lg leading-snug">{prod.name}</h3>
						{#if prod.description}
							<p class="text-stone-500 text-sm mt-1.5 line-clamp-2">{prod.description}</p>
						{/if}
						<div class="flex items-center justify-between mt-4 pt-3 border-t border-stone-100">
							{#if prod.price}
								<span class="text-[#8B5E3C] font-bold text-lg">{fmtPrice(prod.price)}</span>
							{:else}
								<span class="text-stone-400 text-sm">Consultar precio</span>
							{/if}
							<a href={whatsappUrl} target="_blank"
								class="flex items-center gap-1.5 bg-[#25D366] hover:bg-[#1ebe5c] text-white text-xs font-semibold px-3 py-2 rounded-xl transition-colors">
	...
	<div class="max-w-6xl mx-auto px-4 sm:px-8 py-12">
		<div class="grid grid-cols-1 sm:grid-cols-3 gap-8">
			...
			<div>
				...
				<ul class="space-y-2">
					{#each [['#inicio','Inicio'],['#galeria','Galería'], ...(hasCatalog ? [['#catalogo','Catálogo']] : []),['#nosotros','Quiénes somos'],['#contacto','Contacto']] as [href, label]}
